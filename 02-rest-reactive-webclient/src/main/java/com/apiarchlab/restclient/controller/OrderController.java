package com.apiarchlab.restclient.controller;
...
import com.apiarchlab.restclient.service.OrderService;
...
@Slf4j
@Tag(name = "Orders", description = "APIs for managing orders (with async processing)")
public class OrderController {
     ...
     * - Payment processing happens ASYNCHRONOUSLY in background
     * - Request returns immediately without waiting for payment service
     * - Uses WebClient for non-blocking I/O
     * - Notifications are fire-and-forget
     */
    @PostMapping("/{id}/payment")
    @Operation(summary = "Process payment", description = "Process payment for an order (async processing)")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Payment initiated (will process asynchronously)"),
    ...
    public ResponseEntity<PaymentDto> processPayment(
        ...
        log.info("POST /api/orders/{}/payment - Processing payment (ASYNC)", id);
        PaymentDto payment = orderService.processPayment(id, request);
        
        // Note: Payment processing happens in background
        // The actual payment service call (PaymentServiceClient) is async
        // So this response returns quickly while payment processes
